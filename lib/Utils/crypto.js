...
function aesEncryptGCM(plaintext, key, iv, additionalData) {
    ...
    const result = Buffer.allocUnsafe(updated.length + final.length + tag.length);
    updated.copy(result, 0);
    final.copy(result, updated.length);
    tag.copy(result, updated.length + final.length);
    return result;
}
...
function aesDecryptGCM(ciphertext, key, iv, additionalData) {
    ...
    const updated = decipher.update(enc);
    const final = decipher.final();
    const result = Buffer.allocUnsafe(updated.length + final.length);
    updated.copy(result, 0);
    final.copy(result, updated.length);
    return result;
}
...
function aesEncrypt(buffer, key) {
    ...
    const updated = aes.update(buffer);
    const final = aes.final();
    const result = Buffer.allocUnsafe(16 + updated.length + final.length);
    IV.copy(result, 0);
    updated.copy(result, 16);
    final.copy(result, 16 + updated.length);
    return result;
}
